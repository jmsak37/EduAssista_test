...
// Connect to the database to fetch the clarification record.
...
$sql = "SELECT document_upload_link, answer_files, explanation_files FROM clarification WHERE order_id = ?";
...
if (empty($fileParam)) {
    ...
    if (empty($files)) {
        ...
        // Display in a table (like the All Documents view)
        echo "<table>";
        echo "<tr><th>Document Name</th><th>Source</th><th>Action</th></tr>";
        // For types other than 'all', the $files array is simple. We'll loop accordingly.
        if ($type !== 'all') {
            foreach ($files as $f) {
                ...
                echo "<td><a class='button' href='preview_document.php?type=$type&order_id=$order_id&file=" . urlencode($f) . "&source=$source'>Preview</a></td>";
                echo "</tr>";
            }
        } else {
            // 'all' mode already builds an array of arrays.
            foreach ($files as $item) {
                ...
                echo "<td><a class='button' href='preview_document.php?type=$src&order_id=$order_id&file=" . urlencode($f) . "&source=$src'>Preview</a></td>";
                echo "</tr>";
            }
        }
        echo "</table>";
