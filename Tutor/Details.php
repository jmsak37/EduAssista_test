...
// Try to get order_id from GET
$order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;

// If not provided in GET and if there's an opener, try to get it from the opener's order details table
if (!$order_id && isset($_SERVER['HTTP_REFERER']) && !empty($_SERVER['HTTP_REFERER']) && isset($_SESSION['from_beingdone'])) {
    // Alternatively, you might set a session variable from beingdone.html
    $order_id = intval($_SESSION['from_beingdone']);
}

// If still no order id, output error and close
if (!$order_id) {
    echo "Order details not found. Closing window.";
    exit();
}
...
// Retrieve order details from orders table (adjust the query as needed)
$stmt = $conn->prepare("SELECT order_id, client_id, subject, name, description, instructions, deadline, number_of_pages, document_upload_link FROM orders WHERE order_id = ?");
...
<head>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      // Order details are already embedded via PHP.
      // For debugging, log the order id from the hidden input.
      console.log("Order ID from PHP:", document.getElementById("orderIdInput").value);
      
      // Set up toggling for "Other" checkboxes to show corresponding text input.
      ...
      // Update visibility of the Submit Reason button when any option or text input is filled.
      ...
      document.getElementById("skipForm").addEventListener("submit", function(e) {
        ...
        // Build payload with the hidden order ID.
        let payload = {
          order: { order_id: document.getElementById("orderIdInput").value },
...
<body>
  <div class="skip-container">
    <h2>Skip Order</h2>
    <!-- Order Details Section (displayed from PHP) -->
    <div class="order-details-container" id="orderDetails">
      <table class="order-details-table">
        <tbody>
          ...
          <tr>
            <th>Client ID</th>
            <td><?php echo htmlspecialchars($orderData['client_id']); ?></td>
          </tr>
          <tr>
            <th>Subject</th>
            <td><?php echo htmlspecialchars($orderData['subject']); ?></td>
          </tr>
          <tr>
            <th>Name</th>
          ...
          <tr>
            <th>Description</th>
            <td><?php echo nl2br(htmlspecialchars($orderData['description'])); ?></td>
          </tr>
          <tr>
            <th>Instructions</th>
            <td><?php echo nl2br(htmlspecialchars($orderData['instructions'])); ?></td>
          </tr>
          <tr>
            <th>Deadline</th>
            <td><?php echo htmlspecialchars($orderData['deadline']); ?></td>
          </tr>
          <tr>
            <th>Price</th>
            <td>$<?php echo htmlspecialchars($orderData['number_of_pages'] * 2); ?></td>
          </tr>
          <tr>
            <th>Number of Pages</th>
            <td><?php echo htmlspecialchars($orderData['number_of_pages']); ?></td>
          </tr>
        </tbody>
      </table>
      <?php if (!empty($orderData['document_upload_link'])): ?>
      <div class="document-container">
        <h3>Document Download Links</h3>
        <iframe src="<?php echo '../client/' . htmlspecialchars($orderData['document_upload_link']); ?>" width="100%" height="500px" style="border:1px solid #ccc;"></iframe>
        <br>
        <a href="<?php echo '../client/' . htmlspecialchars($orderData['document_upload_link']); ?>" target="_blank">Download Document</a>
      </div>
      <?php endif; ?>
    </div>
    <!-- Hidden input to store Order ID -->
    <input type="hidden" id="orderIdInput" name="order_id" value="<?php echo htmlspecialchars($orderData['order_id']); ?>">
    
    <!-- Skip Order Form Section -->
